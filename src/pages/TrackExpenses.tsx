...
const TrackExpenses = () => {
  ...
  const [selectedMembersForSplit, setSelectedMembersForSplit] = useState<
    string[]
  >([]);
  ...
  const spendByDate = filteredExpenses.reduce(
    (acc, exp) => {
  ...
  const handleChatExpenseSubmit = () => {
    ...
    const dateMatch = chatInput.match(
      /\s+on\s+(\w+\s+\d{1,2}(?:,\s*\d{4})?)$/i,
    );
  ...
  const toCamelTrip = (t: Record<string, unknown>): TripTemplate => ({
    ...
    moneyHandler: t.money_handler as string,
    location: t.location as string,
    expenseTypes: t.expense_types as string[],
    expenseTypeOptions: t.expense_type_options as Record<string, string[]>,
  ...
  const getTripStatus = (trip: TripTemplate) => {
    const now = new Date();
  ...
  const exportToPDF = () => {
    ...
    doc.text(
      `Remaining Budget: Rs. ${(selectedTrip.budget - totalAmount).toFixed(2)}`,
      14,
      40,
    );
    ...
    autoTable(doc, {
      ...
      body: [
        ...expenses.map((e) => [
          ...
          ...selectedTrip.members.map(
            (m) => `Rs. ${(e.memberAmounts[m] || 0).toFixed(2)}`,
          ),
          e.createdBy || "N/A",
        ]),
        [
        ...
        [
          ...
          "",
          "Settlement",
          `Rs. ${(selectedTrip.budget - totalAmount).toFixed(2)}`,
          ...selectedTrip.members.map((m) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_BASE}/expenses?tripId=${selectedTripId}`).then((res) =>
        res.json(),
      ),
      fetch(`${API_BASE}/logs?tripId=${selectedTripId}`).then((res) =>
        res.json(),
      ),
    ])
  ...
  const sortedTrips = [...tripTemplates].sort((a, b) => {
    const getPriority = (trip: TripTemplate) => {
      const status = getTripStatus(trip);

      if (status.includes("Planned")) return 0;
      if (status.includes("Active")) return 1;
  ...
  const splitAmongSelectedMembers = () => {
    ...
    toast({
      title: "Amount Split",
      description: `₹${total} split among selected`,
    });
  };
  ...
  const getExpenseOptions = () =>
    selectedTrip?.expenseTypeOptions[expenseType] || [];
  ...
  const deleteExpense = (id: string) => {
    ...
    fetch(`${API_BASE}/expenses/${id}`, { method: "DELETE" }).then(() => {
      toast({ title: "Deleted", description: "Expense deleted" });
      setExpenses((prev) => prev.filter((e) => e.id !== id));

      const logEntry =
        `🔴 ${user?.displayName || "Someone"} deleted expense\n` +
  ...
  const statCard = (
    ...
    >
      <span
        className="text-xs uppercase tracking-widest font-mono-custom"
  ...
  const budgetPct = selectedTrip
    ? Math.min((totalAmount / selectedTrip.budget) * 100, 100)
    : 0;
  ...
  return (
    ...
    >
      ...
      <main className="max-w-[1600px] mx-auto px-10 py-10 flex-1 w-full space-y-6">
        {/* Trip selector or loading */}
        {loadingTrips ? (
          <div className="space-y-3">
            {[1, 2, 3].map((n) => (
              <div key={n} className="h-16 rounded-2xl loading-skeleton" />
            ))}
          </div>
        ) : !selectedTrip ? (
  ...
  "
          ...
          <div className="space-y-3">
            {[1, 2, 3].map((n) => (
              <div key={n} className="h-20 rounded-2xl loading-skeleton" />
            ))}
          </div>
        ) : (
          <div className="fade-up space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <ActionBtn
                ...
                icon="💸"
                label={showSettlement ? "Hide Settlement" : "Settle"}
                color="var(--sage)"
              />
              <ActionBtn
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filteredExpenses.map((exp, ri) => (
                        ...
                        >
                          <td
                            className="px-4 py-3 font-mono-custom text-xs whitespace-nowrap"
                            style={{ color: "var(--text-secondary)" }}
                          >
                          ...
                          <td
                            className="px-4 py-3 font-mono-custom font-semibold whitespace-nowrap"
                            style={{ color: "var(--amber)" }}
                          >
                          ...
                          {selectedTrip.members.map((m) => (
                            <td
                              key={m}
                              className="px-4 py-3 font-mono-custom text-xs text-center"
                              style={{ color: "var(--text-muted)" }}
                            >
                      ...
                      >
                        ...
                        {selectedTrip.members.map((m) => {
                          const mt = filteredExpenses.reduce(
                            (s, e) => s + (e.memberAmounts[m] || 0),
                            0,
                          );
                          return (
                            <td
                              key={m}
                              className="px-4 py-3 font-mono-custom text-xs text-center font-bold"
                              style={{ color: "var(--text-secondary)" }}
                            >
                              ₹{mt.toFixed(2)}
                            </td>
                      ...
                      {showSettlement && (
                        ...
                        >
                          ...
                          {selectedTrip.members.map((m) => {
                            ...
                            const share =
                              selectedTrip.budget / selectedTrip.members.length;
                    ...
                    <thead>
                      ...
                      >
                        ...
                        {selectedTrip.expenseTypes.map((t) => (
                          <th
                            key={t}
                            className="px-4 py-3 text-right text-xs uppercase tracking-widest"
                            style={{ color: "var(--text-muted)" }}
                          >
                            {t}
                          </th>
                        ...
                        return (
                          ...
                          >
                            ...
                            {selectedTrip.expenseTypes.map((t) => (
                              <td
                                key={t}
                                className="px-4 py-3 font-mono-custom text-xs text-right"
                                style={{ color: "var(--text-secondary)" }}
                              >
                            ...
                            <td
                              className="px-4 py-3 font-mono-custom text-sm text-right font-bold"
                              style={{ color: "var(--terracotta)" }}
                            >
                              ₹{total.toFixed(2)}
                            </td>
      ...
      {showChatToAdd && (
        ...
        >
          <div
            className="rounded-2xl w-full max-w-lg p-6 space-y-4 fade-up"
            style={{
              background: "var(--bg-card)",
              border: "1px solid var(--border)",
            }}
          >
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={() => setShowChatToAdd(false)}
            ...
            <button
              onClick={() =>
                setChatInput(
                  "Spent ₹1200 on Stay as advance at Taj for everyone",
                )
              }
      ...
      {showExpenseDrawer && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          />
          <div
            className="relative w-full sm:w-[420px] h-full overflow-y-auto slide-in"
            style={{
              background: "var(--bg-card)",
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {expenseType && getExpenseOptions().length > 0 && (
                  <div>
                    ...
                    <Select
                      value={expenseOption}
                      onValueChange={setExpenseOption}
                    >
                      <SelectTrigger
                        style={{
                          background: "var(--bg-elevated)",
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Popover
                      ...
                      onOpenChange={setShowMemberSelection}
                    >
                      <PopoverTrigger asChild>
                        <button
                          type="button"
                      ...
                      >
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {selectedTrip?.members.map((m) => (
                              <div key={m} className="flex items-center gap-2">
                                <Checkbox
                                  id={`split-${m}`}
                                  checked={selectedMembersForSplit.includes(m)}
                                  onCheckedChange={() =>
                          ...
                          <button
                            ...
                            disabled={!selectedMembersForSplit.length}
                  ...
                  <p
                    ...
                    style={{ color: "var(--text-muted)" }}
                  >
                    <Users className="inline h-3 w-3" /> = equal split
                ...
                <div>
                    ...
                    {selectedTrip?.members.map((m, i) => (
                      <div key={i}>
                        ...
                        <input
                          type="number"
                          step="0.01"
                          value={memberAmounts[m] || 0}
                          onChange={(e) =>
                            handleMemberAmountChange(m, e.target.value)
                          }
                          placeholder="0.00"
                  ...
                  <p
                    className="text-xs mt-2 font-mono-custom"
                    style={{ color: "var(--text-secondary)" }}
                  >
                    Assigned: ₹
      ...
      {showActivityDrawer && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          />
          <div
            className="relative w-full sm:w-[400px] h-full overflow-y-auto slide-in"
            style={{
              background: "var(--bg-card)",
              borderLeft: "1px solid var(--border)",
            }}
          >
            ...
            <div className="p-6">
              {activityLogs.length === 0 ? (
                <p
                  className="text-sm italic text-center py-12"
                  style={{ color: "var(--text-muted)" }}
                >
                  No activity yet.
                </p>
                    ...
                    >
                      ...
                      {log.split_details && (
                        <div className="mt-2 space-y-0.5">
                          {Object.entries(log.split_details).map(
                            ([name, amt]) => (
                              <div
                                key={name}
